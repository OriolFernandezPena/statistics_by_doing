...
**Theorem (CLT)** Let $X_1, X_2, ...$ be iid random variables such that $\mathbb{E}(X_i) = \mu < \infty$ and $Var(X_i) = \sigma^2 < \infty$ for all $i$. Let $$\overline{X}_ n := \frac{1}{n} \sum_{i=1}^{n} X_i$$ and $$Z_n := \frac{\overline{X}_n - \mu}{\frac{\sigma}{\sqrt{n}}}.$$
Then
$$\overline{X}_n \xrightarrow{d} X \sim \mathcal{N}(\mu, \frac{\sigma^2}{n}),$$
